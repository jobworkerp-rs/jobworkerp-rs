...
// /// Listen to job result by job_id and return a static-lifetime stream of JSON values.
// ///
// /// This is a standalone function that takes owned/Arc parameters, suitable for use
// /// in spawned tasks that require 'static lifetime.
// ///
// /// # Arguments
// /// * `function_app` - Arc to FunctionAppImpl (or any FunctionCallHelper)
// /// * `job_id` - The job ID to listen for (owned)
// /// * `runner_name` - Runner name for decoding output (owned)
// /// * `using` - Optional method name for multi-method runners (owned)
// /// * `timeout` - Optional timeout in milliseconds
// ///
// /// # Returns
// /// Stream of JSON values representing the job result chunks with 'static lifetime
// pub fn listen_job_result_as_json_stream_static<F>(
//     function_app: std::sync::Arc<F>,
//     job_id: proto::jobworkerp::data::JobId,
//     runner_name: String,
//     using: Option<String>,
//     timeout: Option<u64>,
// ) -> std::pin::Pin<Box<dyn futures::Stream<Item = Result<serde_json::Value>> + Send + 'static>>
// where
//     F: FunctionCallHelper + Send + Sync + 'static,
// {
//     use futures::StreamExt;
//     use proto::jobworkerp::data::result_output_item;
...
//                         Some(result_output_item::Item::Data(data)) => {
//                             match function_app
//                                 .decode_job_result_output(None, Some(&runner_name), &data, using.as_deref())
//                                 .await
//                             {
//                                 Ok(json) => yield Ok(json),
//                                 Err(e) => {
