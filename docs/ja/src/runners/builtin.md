...
| GRPC | gRPC通信（マルチメソッド） | gRPC unaryおよびサーバストリーミングリクエスト | worker.runner_settings: host, port, TLS設定, デフォルトmethod/metadata/timeout/as_json（すべて任意）等, job.args: method, request, metadata, timeout, as_json（settings側で設定時はすべて省略可）, using: "unary" or "streaming" |
