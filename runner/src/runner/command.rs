...
use std::{
    future::Future, mem, process::Stdio, sync::{Arc, Mutex}, time::{Duration, Instant, SystemTime, UNIX_EPOCH}
};
use sysinfo::System;
...
impl RunnerTrait for CommandRunnerImpl {
    ...
    async fn run(&mut self, args: &[u8]) -> Result<Vec<Vec<u8>>> {
        ...
        tracing::info!("run command: {}, args: {:?}, monitor_memory: {}, started_at: {}", 
            &data.command, args, should_monitor_memory, started_at);
    ...
    async fn run_stream(&mut self, args: &[u8]) -> Result<BoxStream<'static, ResultOutputItem>> {
        let data =
            ProstMessageCodec::deserialize_message::<CommandArgs>(args).context("on run_stream job")?;
        ...
        tracing::info!("run_stream command: {}, args: {:?}, monitor_memory: {}, started_at: {}", 
            &command_str, &args_vec, should_monitor_memory, started_at);
...
mod tests {
    ...
    async fn test_run() {
        ...
        let result = ProstMessageCodec::deserialize_message::<CommandResult>(&result_bytes).unwrap();
    ...
    async fn test_run_stream() {
        ...
        while let Some(item) = stream.next().await {
            match item.item {
                Some(Item::Data(data)) => {
                    ...
                    assert!(result.is_ok(), "Should be able to deserialize the data as CommandResult");
        ...
        assert!(found_stdout_data, "Stream should produce CommandResult with stdout data");
        assert!(found_final_result, "Stream should produce a final CommandResult with exit code and execution time");
    ...
    async fn test_run_stream_with_multiple_lines() {
        ...
        tokio::spawn(async move {
                ...
                match item.item {
                    Some(Item::Data(data)) => {
                        ...
                        if let Some(stdout) = result.stdout {
                            ...
                            // This is the final result packet
                            eprintln!("[{}ms] RESULT: exit_code={:?}, execution_time={}ms", 
                                elapsed, result.exit_code, result.execution_time_ms.unwrap_or(0));
                            io::stderr().flush().unwrap();
            ...
            eprintln!("Stream processing completed: lines={}, final_result={}, end_marker={}", 
        ...
        if let Some((found_final_result, found_end, line_count)) = rx.recv().await {
            // Verify results
            assert!(line_count >= 3, "Stream should capture at least 3 lines of output");
            assert!(found_final_result, "Stream should produce a final CommandResult");
            assert!(found_end, "Stream should produce an end marker");
