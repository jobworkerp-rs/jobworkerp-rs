...
impl WorkflowLoader {
    const WORKFLOW_VALIDATOR: OnceCell<Option<jsonschema::Validator>> = OnceCell::const_new();
    ...
    async fn validate_schema(&self, instance: &serde_json::Value) -> Result<()> {
        ...
        if let Some(validator) = Self::WORKFLOW_VALIDATOR
            .get_or_init(|| async {
                ...
                match std::thread::spawn(move || {
                    jsonschema::draft202012::new(&schema).inspect_err(|e| {
                        tracing::warn!("Failed to create workflow schema validator: {:?}", e)
                    })
                })
                .join()
                {
                    Ok(validator_result) => validator_result.ok(),
                    Err(e) => {
                        tracing::warn!("Thread panicked while creating validator: {:?}", e);
                        None
                    }
                }
            })
