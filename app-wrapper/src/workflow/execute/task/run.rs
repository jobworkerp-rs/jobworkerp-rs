...
impl TaskExecutorTrait<'_> for RunStreamTaskExecutor {
    async fn execute(
        ...
        // Handle Worker configuration (streaming version)
        match run {
            workflow::RunTaskConfiguration::Worker(workflow::RunWorker {
                worker:
                    RunJobWorker {
                ...
                // Use streaming collection method
                let output = match self
                    .job_executor_wrapper
                    .enqueue_with_worker_name_and_collect_stream(
                        metadata,
                        worker_name,
                        &args,
                        None,
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        ...
                        let pos = pos.read().await.as_error_instance();
                        Err(workflow::errors::ErrorFactory::new().service_unavailable(
                            "Failed to execute streaming job by jobworkerp".to_string(),
                            Some(pos),
                            Some(format!("{e:?}")),
                        ))
                    }
