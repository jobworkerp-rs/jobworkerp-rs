...
use jobworkerp_runner::runner::workflow::SavedWorkflowRunnerSpec;
...
#[derive(Debug, Clone)]
pub struct SavedWorkflowRunner {
    app_module: Arc<AppModule>,
    workflow_executor: Option<WorkflowExecutor>,
    canceled: bool,
}
impl SavedWorkflowRunner {
...
impl SavedWorkflowRunnerSpec for SavedWorkflowRunner {}
...
impl RunnerSpec for SavedWorkflowRunner {
    fn name(&self) -> String {
...
#[async_trait]
impl RunnerTrait for SavedWorkflowRunner {
    ...
    async fn run(&mut self, args: &[u8]) -> Result<Vec<Vec<u8>>> {
        ...
        if self.canceled {
            return Err(anyhow::anyhow!(
                "canceled by user: {}, {:?}",
                RunnerType::SavedWorkflow.as_str_name(),
                arg
            ));
        }
