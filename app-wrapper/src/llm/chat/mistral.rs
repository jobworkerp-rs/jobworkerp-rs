...
pub struct MistralChatService {
    ...
    pub converter: DefaultLLMRequestConverter,
}
...
impl MistralChatService {
    pub async fn new(
        ...
        let converter = DefaultLLMRequestConverter::new(Some(function_app.clone()));
        Ok(Self {
            service,
            function_app: Some(function_app),
            converter,
        })
    }
    ...
    pub async fn request_chat(
        ...
        if self.has_function_calling(&args) {
            ...
            let request_builder = self.converter.build_request(&args, false).await?;
    ...
    async fn execute_tool_call(
        ...
        if let Some(function_app) = &self.function_app {
            tracing::debug!(
            ...
            tracing::warn!("No function app available for tool execution");
            format!(
                "Tool '{}' not available: no function app configured",
                tool_call.fn_name
            )
        }
    }
    ...
    pub async fn request_stream_chat(
        ...
        let request_builder = self.converter.build_request(&args, true).await?;
