...
- `call`タスク: `call: http` の基本的なHTTP呼び出し（method, endpoint, headers, query, body, output）を採用
...
- `return` は process result を生成する shell/container 専用です。runner / function / worker / workflow は runner 定義の出力型を持つため `return` を拒否します。`return` は `stdout`、`stderr`、`code`、`all`、`none` を受け付け、`return: all` は `{code, stdout, stderr}` を出力します。
...
- **外部サービス呼び出し（call task）**: `call: http` は対応しています。`call: grpc`、AsyncAPI、OpenAPI、named function call、endpoint-level authentication、HTTP `output: raw` は未対応です
- **await / return の適用範囲**: `await` はすべての `run.*` インスタンスで利用できます（`await: false` = fire-and-forget）。`return` は shell/container 専用で、runner / function / worker / workflow は拒否します。`useStreaming: true` の場合、すべてのインスタンスで `await: false` は拒否されます
