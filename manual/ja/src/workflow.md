...
- `run.shell` / `run.container` は公式 process の `await` / `return` に対応します。`return` は `stdout`、`stderr`、`code`、`all`、`none` を受け付けます。`return: all` は `{code, stdout, stderr}` を出力します。`await: false` はプロセス開始後に完了を待たず、現在のタスク入力をタスク出力にして続行します。
...
- `run.script` / `run.workflow` は `await` / `return` を受け付けません。
...
- **await / return の適用範囲**: `run.shell` / `run.container` は公式 process の `await` / `return` 動作に対応します。`run.script` / `run.workflow` はこれらの field に対応しません
