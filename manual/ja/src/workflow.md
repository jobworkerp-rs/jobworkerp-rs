...
- `call`タスク: `call: http` の基本的なHTTP呼び出し（method, endpoint, headers, query, body, output）を採用。endpoint の `authentication`（bearer / basic）にも対応
...
- **外部サービス呼び出し（call task）**: `call: http` は対応しています（endpoint の `authentication` は `bearer` / `basic` scheme に対応）。`call: grpc`、AsyncAPI、OpenAPI、named function call、`digest` / `oauth2` / `oidc` / `certificate` scheme、HTTP `output: raw` は未対応です
