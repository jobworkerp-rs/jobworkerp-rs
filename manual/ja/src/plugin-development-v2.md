...
> **注意**: **サーバー安定性に関する警告 (V1 と共通)**
> プラグインはホストプロセスに動的ライブラリとしてロードされます。プラグイン
> 内で **panic** が発生すると **サーバ全体がクラッシュ** します。
> - `unwrap()` / `expect()` の使用は避け、`Result` で返してください。
> - 入力値は丁寧に検証してください。
> - 初期化失敗(例: `tokio::runtime::Runtime::new()`)は `new()` ではなく
>   `load()` 内で扱い、`Err` を返してください。
...
**tokio runtime 非依存** (`Handle::current()` を呼ばない) なので、host
runtime で作成した sender を plugin runtime 上で `.send().await` しても問題
ありません。これは `CancellationToken` と同じく「FFI 境界を越えて渡せる
tokio の同期プリミティブ」のひとつです。ただし `tokio` クレートのバージョン
は host と plugin で完全一致させてください。
