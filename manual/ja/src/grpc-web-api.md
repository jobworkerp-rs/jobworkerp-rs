...
service FunctionService {
  ...
  // FunctionUsing（FunctionId + optional using）による検索
...
以下のフィールドをRunner Settingsでデフォルト値として設定できます。設定時はジョブ引数（GrpcArgs）の対応フィールドより優先されます：

- `connection_timeout`（任意）: 接続タイムアウト（ミリ秒）。gRPCチャネル確立時に使用されます。
- `method`（任意）: デフォルトのgRPCメソッド（"service/method"形式）。同一エンドポイントに繰り返しリクエストするワーカーで有用です。
- `metadata`（任意）: デフォルトのリクエストメタデータ（キー・値ペア）。Args側のmetadataとマージされ、同一キーはsettings側が優先されます。
- `timeout`（任意）: デフォルトのリクエストタイムアウト（ミリ秒）。`connection_timeout`（チャネル確立用）とは異なります。
- `as_json`（任意）: レスポンスをJSONに変換するかのデフォルト値。`use_reflection` または `proto` が必要です。
- `proto`（任意）: リフレクションを使わずにJSONとprotobufを変換するための `.proto` 定義です。`source` には `https://` URI、`file://` URI、またはinline proto文字列を指定できます。`http://` は `allow_insecure_http: true` の場合のみ許可されます。`fetch_headers` はHTTP(S)取得時に送信されます。`file://` は環境変数 `GRPC_PROTO_ALLOWED_DIR` に許可ディレクトリを設定した場合のみ使用できます。

`proto` を指定した場合、Runnerは起動時または実行時にproto descriptorを構築してキャッシュします。worker実行環境には `protoc` が必要です。Runner Settings側の `proto` はJob Arguments側の `proto` より優先されます。
...
- `method`, `metadata`, `timeout`, `as_json`, `proto` はすべてrunner settingsで対応フィールドが設定されている場合は省略可能。両方設定されている場合はsettings側が優先されます。
...
**Job Arguments (args_json) - Ollama completionの場合:**
```json
{
...
**Job Arguments (args_json) - GenAI chat（`chat`メソッド使用時）:**
```json
{
  "messages": [
...
> **注意**: メソッド選択（`"completion"`/`"chat"`）はgRPC直接利用時に`JobRequest.using`で指定します。FunctionService.Call経由では、`runner_name___method_name`形式（トリプルアンダースコア区切り）でメソッドを指定できます（例: `llm___chat`）。指定しない場合はデフォルトの`run`メソッドが実行されます。

### FUNCTION_SET_SELECTOR - FunctionSet一覧（内部利用）

LLMのAutoSelection機能で使用される内部Runner。利用可能なFunctionSetをツール概要付きで一覧表示します。通常はユーザーが直接利用することはありません。

### WORKFLOW - ワークフロー実行

WORKFLOWランナーはマルチメソッドランナーです。実行メソッド（`run`または`create`）は、gRPC直接利用時に`JobRequest.using`で指定するか、FunctionService.Call経由では`runner_name___method_name`形式（例: `workflow___run`）で指定します。
...
**再利用可能ワークフローWorkerの作成 (using: "create"):**
...
- [protobuf スキーマ定義](https://github.com/jobworkerp-rs/jobworkerp-rs/tree/main/proto/protobuf/jobworkerp/function/)
