# JobWorkerP プラグイン開発ガイド (V1)

このガイドでは、Rustを使用してJobWorkerPのプラグインを作成する方法を説明します。プラグインを使用することで、独自のジョブ実行ロジック（ランナー）を実装し、JobWorkerPの機能を拡張することができます。

> **新規プラグインを書く場合**: より新しい **V2 trait** (async-ffi ベース、`CancellationToken` による協調的キャンセル、push 型ストリーミング) も利用可能です。詳細は [プラグイン開発 (V2 / async-ffi)](./plugin-development-v2.md) を参照してください。V1 と V2 は同一ホスト上で並存可能で、既存 V1 プラグインのバイナリ互換性は維持されます。
...
`build.rs`を設定してprotoファイルをコンパイルします。`prost_build`を使用します。プラグインはgRPCサービスを定義しないため、`tonic-prost-build`は不要です。
