...
- `call` task: Basic `call: http` support (method, endpoint, headers, query, body, output) including endpoint `authentication` (bearer / basic)
...
- **External service calls (call task)**: `call: http` is supported, including endpoint `authentication` with the `bearer` and `basic` schemes. `call: grpc`, AsyncAPI, OpenAPI, named function calls, the `digest` / `oauth2` / `oidc` / `certificate` authentication schemes, and HTTP `output: raw` are not supported
