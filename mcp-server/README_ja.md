...
- `LLM`: LLM テキスト生成（using="completion" または "chat"）
- `WORKFLOW`: ワークフロー実行（using="run" または "create"）
- カスタムプラグイン

加えて、登録済みの **Worker** もツールとして公開されます（`MCP_EXCLUDE_WORKER=true` で除外可能）。

## ツール呼び出し形式

引数の形式はツールの対象によって異なり、各ツールの `inputSchema` にそのまま反映されています。スキーマどおりに引数を渡せば正しく動作します。

- **Runner ツール（直接実行）**: 初期化設定と実行引数の両方が必要なため `{ "settings": {...}, "arguments": {...} }` 形式。

  ```json
  { "arguments": { "command": "echo", "args": ["Hello, World!"] } }
  ```

- **Worker ツール（事前設定済み）**: 設定は Worker 作成時に確定済みのため、引数を**トップレベルに直接**指定（`settings`/`arguments` ラップなし）。WORKFLOW Worker の場合はワークフローの `input` フィールドをトップレベルに直接指定します（jobworkerp が自動的にワークフローの `input` へ包みます）。

  ```json
  { "owner": "jobworkerp-rs", "repo": "jobworkerp-rs" }
  ```

  WORKFLOW Worker の `run` 以外のメソッド（`worker_name___create`）はワークフローの `input` 包装を行わないため、そのメソッドのスキーマ（例: `workflow_data` / `name`）をトップレベルに指定します。
