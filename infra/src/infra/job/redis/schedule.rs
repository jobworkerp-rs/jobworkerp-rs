...
pub trait RedisJobScheduleRepository:
    UseRedisLock + UseJobqueueAndCodec + Send + Sync + 'static
where
    Self: Send + 'static,
{
    ...
    // XXX retry 5 times
    fn _lock_run_after_jobs_with_retry(
        &self,
    ...
    fn _unlock_run_after_jobs(&self) -> impl std::future::Future<Output = Result<()>> + Send {
        async { self.unlock(Self::_LOCK_KEY).await }
    }

    // store job id as zset (using 'zadd' redis command) with epoch msec score (score is run_after_time)
    fn add_run_after_job(&self, job: Job) -> impl std::future::Future<Output = Result<Job>> + Send {
        async {
            let job_id = job
                .id
                .as_ref()
    ...
    fn _find_run_after_jobs_to_run(
        &self,
        now_millis: i64,
    ) -> impl std::future::Future<Output = Result<Vec<Job>>> + Send {
        async move {
    ...
    fn count_all(&self) -> impl std::future::Future<Output = Result<i64>> + Send {
        async {
            let mut redis = self.redis_pool().get().await?;
            redis
    ...
    // for test
    fn delete_all(&self) -> impl std::future::Future<Output = Result<i32>> + Send {
        async {
            let mut redis = self.redis_pool().get().await?;
            redis
